...
const textareaValue = ref("Do Something!!!");
...
const getSubject = async () => {
  try {
    const res = await request.get(
      `${import.meta.env.VITE_API_URL}api/v1/promts/list`
    );
...
//👉 Genarate chat -->
const loadingGenChat = ref(false);
const postChat = async () => {
  loadingGenChat.value = true;
  streamData();
};

//👉 Set Interval stream data chat  -->
async function streamData() {
  const data = JSON.stringify(saveForm.value);
  await request
    .post(`${import.meta.env.VITE_API_URL}api/v1/create/stream`, {
      content: data,
      id_project: ObjectId,
...
async function genSuggestions(textString) {
  await request
    .post(`${import.meta.env.VITE_API_URL}api/v1/create/title`, {
      content: textString,
      id_project: ObjectId,
    })
    .then((res) => {
...
// Đặt một biến để lưu ID của interval
let intervalId;

// Bắt đầu thực thi hàm myFunction sau mỗi 1 giây
function startInterval(res_old) {
  intervalId = setInterval(async () => {
    await request
      .post(`${import.meta.env.VITE_API_URL}api/v1/create/status`, {
        id: res_old.data.data,
      })
      .then((res) => {
        // if(res.data)
        textareaValue.value = res.data.data;
        if (res.data.stream) {
          loadingGenChat.value = false;
          //clear chat 
          //gen images
          genSuggestions(textareaValue.value)
...
// Dừng việc thực thi hàm myFunction định kỳ
function stopInterval(intervalId) {
  clearInterval(intervalId);
}
//👉 Get title Image  -->
const dataTitleImg = ref(null)
const isdataTitleLoading = ref(false)
const GenImage = async (data)=>{
  await request.post(`${import.meta.env.VITE_API_URL}api/v1/create/image`, {
    content: data,
    id_project: ObjectId,
  }
  ).then((res)=>{
...
function waitRenderTitle(res_old) {
  isdataTitleLoading.value = true
  var idSetInterval = setInterval(async () => {
    await request
      ...
      .then((res) => {
        // if(res.data)
        if (res.data.msg != 'processing') {
          var data = JSON.parse(res.data.data)
          var lineImage = data.choices[0].message.content
          var splitData = lineImage.split('\n\n')
          dataTitleImg.value = splitData
          loadingGenChat.value = false;
          isdataTitleLoading.value = false
          //clear chat 
    ...
    <VRow>
      <VCol cols="4" sm="4" v-model="panel">
        ...
        <Vrow>
          ...
          >
            <VBtn v-if="!loadingGenChat" @click="postChat" variant="flat">
              Gen Chat
            </VBtn>
            <VBtn v-if="loadingGenChat" color="success" size="large">
              Waiting... <VProgressCircular indeterminate color="warning" />
            </VBtn>
          </VCol>
        </Vrow>
      </VCol>
      ...
      <VCol cols="6" sm="6" style="height: 780px">
        <div class="scrollable-wrapper">
          ...
          <VRow style="margin: 0 !important">
            <a-empty v-if="!dataTitleImg">
              <template #description>
                <span> No data Image </span>
              </template> 
              <a-spin v-if="isdataTitleLoading" />
            </a-empty> 
            <VCol
            v-if="dataTitleImg"
              cols="6"
              sm="6"
              v-for="(item, index) in dataTitleImg"
              :key="index"
            >
              <a-image
                v-if="item.src"
              ...
              <div v-else class="empty-img">
                <VBtn class="btn-genImage" variant="tonal" @click="GenImage"> Generate </VBtn>
                <a-image
      ...
      <VCol cols="2" sm="2">
        <div class="gen-all">
          <VBtn style="padding: 3px; min-height: 42px">
            Video & Âm thanh
